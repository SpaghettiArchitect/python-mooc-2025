...
def sort_by_number_of_routes(areas: list):
    def get_number_of_routes(area: ClimbingArea):
        return area.routes()

    return sorted(areas, key=get_number_of_routes)


def sort_by_most_difficult(areas: list):
    def get_area_difficulty(area: ClimbingArea):
        return area.hardest_route().grade

    return sorted(areas, key=get_area_difficulty, reverse=True)
